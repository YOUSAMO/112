...
public class AdminController {
    ...
    private final LostFoundAnimalService lostFoundAnimalService;
    ...
    private void setCommonModelAttributes(Model model, HttpSession session) {
        ...
        model.addAttribute("adoptionreview",adoptionReviewService.getAllReviews());
        model.addAttribute("lostfoundanimals",lostFoundAnimalService.getAllLostFoundAnimals());
    ...
    // 자유게시판
    @PostMapping("/delete/board/{bNo}")
    public String deleteBoard(@PathVariable Long bNo) {
        boardService.deleteById(bNo);
        return "redirect:/admin/adminPage"; // 관리자 메인으로 리다이렉트
    }

    // 입양후기 게시판
    @PostMapping("/delete/review/{arNo}")
    public String deleteReview(@PathVariable Long arNo) {
        adoptionReviewService.deleteById(arNo);
        return "redirect:/admin/adminPage";
    }

    // 유기동물/실종 보호 게시판
    @PostMapping("/delete/missing/{id}")
    public String deleteMissing(@PathVariable Long id) {
        lostFoundAnimalService.deleteByid(id);
        return "redirect:/admin/adminPage";
    }

    // 댓글 관리
    @PostMapping("/delete/comment/{cmNo}")
    public String deleteComment(@PathVariable Long cmNo) {
        commentService.deleteBycmNo(cmNo);
        return "redirect:/admin/adminPage";
    }
